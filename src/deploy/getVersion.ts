...
export async function getVersion(packageName: string, bumpType: ReleaseType = 'patch'): Promise<string> {
      ...
      res.on('end', () => {
        try {
          ...
          if (!json['dist-tags']?.latest) {
            console.error(`No 'latest' version found for package: ${packageName}`);
            resolve('0.0.1');
            return;
          }
          ...
          console.error(`Error parsing JSON for package ${packageName}:`, error);
